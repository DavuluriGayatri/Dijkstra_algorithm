...
def create_map_canvas():
    ...
    cv2.fillPoly(canvas, [hexagon_pts], color=(255, 255, 255))
    # Obstacle 4 (Rectangular C-shaped obstacle)
    c_shape_rect_pts = np.array([[900, 450], [1100, 450], [1100, 50], [900, 50], [900, 125], [1020, 125], [1020, 375], [900, 375]], dtype=np.int32)
    cv2.fillPoly(canvas, [c_shape_rect_pts], color=(255, 255, 255))
    cv2.polylines(canvas, [c_shape_rect_pts], isClosed=True, color=(255, 255, 255), thickness=1)
...
if __name__ == "__main__":
    ...
    ax.imshow(cv2.cvtColor(map_canvas, cv2.COLOR_BGR2RGB), extent=[0, MAP_WIDTH, 0, MAP_HEIGHT])
